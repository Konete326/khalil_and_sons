<x-layouts.admin :title="'Commission Inspection: ' . $order->tracking_code . ' | Khalil & Sons'">
    <div class="space-y-6 max-w-5xl" x-data="{ zoomModal: false, rejectModal: false }">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gold-antique/20 pb-4">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('track', ['code' => $order->tracking_code]) }}" target="_blank" class="border border-gold-antique/40 px-3.5 py-2 text-xs font-mono text-gold-light hover:bg-gold-antique hover:text-oxblood-dark transition">Customer View</a>
                <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $order->customer_phone) }}?text={{ urlencode('Assalam-o-Alaikum ' . $order->customer_name . ', regarding your bespoke order ' . $order->tracking_code) }}" target="_blank" class="border border-[#25D366]/60 bg-[#25D366]/10 px-3.5 py-2 text-xs text-[#25D366] hover:bg-[#25D366] hover:text-black transition">WhatsApp Patron</a>
            </div>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="border border-gold-antique/30 bg-oxblood-dark/90 p-5 space-y-4 shadow-xl text-xs font-mono">
                ...
                @if($order->notes)
                    <div class="p-3 border border-gold-antique/15 bg-black/40 text-[11px] text-ivory-base/80">
                ...
                <div class="border-t border-gold-antique/20 pt-3">
                    ...
                    <img src="{{ asset($order->original_image_path) }}" alt="Design Reference" class="w-full aspect-square object-cover border border-gold-antique/30 bg-black/50" />
                </div>
            </div>
            ...
            <div class="border border-gold-antique/30 bg-oxblood-dark/90 p-5 space-y-4 shadow-xl text-xs font-mono">
                <div class="flex items-center justify-between border-b border-gold-antique/20 pb-2">
                    ...
                    <span class="text-[10px] uppercase px-2 py-0.5 {{ $order->payment_status === 'verified' ? 'bg-emerald-950 text-emerald-300' : ($order->payment_status === 'slip_uploaded' ? 'bg-amber-950 text-amber-300 animate-pulse' : 'bg-black/50 text-ivory-base/60') }}">{{ $order->payment_status }}</span>
                </div>
                @if($order->payment_slip_path)
                    <div class="relative cursor-pointer group" @click="zoomModal = true">
                        <img src="{{ asset($order->payment_slip_path) }}" alt="Customer Slip" class="w-full aspect-[3/4] object-cover border border-gold-antique/40 shadow-inner" />
                        <div class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center text-gold-light transition text-xs font-sans">Click to Zoom Lightbox</div>
                    </div>
                    <div class="flex gap-2 pt-2">
                        <form action="{{ route('admin.orders.payment', $order) }}" method="POST" class="flex-1">
                            @csrf
                            <input type="hidden" name="action" value="verify" />
                            <button type="submit" class="w-full bg-emerald-600 hover:bg-emerald-500 py-2.5 text-xs font-semibold uppercase text-black transition">Verify Slip</button>
                        </form>
                        <button type="button" @click="rejectModal = true" class="flex-1 border border-red-500/60 bg-red-950/40 hover:bg-red-900 py-2.5 text-xs font-semibold uppercase text-red-300 transition">Reject</button>
                    </div>
                @else
                    <div class="py-12 text-center text-ivory-base/50 border border-dashed border-gold-antique/20 p-4">
            ...
            <div class="border border-gold-antique/30 bg-oxblood-dark/90 p-5 space-y-4 shadow-xl text-xs font-mono">
                ...
                <div class="space-y-2 text-xs">
                    ...
                    <div class="p-3 border border-gold-antique/30 bg-black/40 text-gold-light font-bold text-sm uppercase">
                ...
                <form action="{{ route('admin.orders.stage', $order) }}" method="POST" class="space-y-3 pt-3 border-t border-gold-antique/15">
                    ...
                    <select name="manufacturing_status" class="w-full border border-gold-antique/30 bg-black/50 p-2.5 text-xs text-ivory-base focus:border-gold-antique focus:outline-none">
                    ...
                    <button type="submit" class="w-full bg-gold-antique py-2.5 text-xs font-semibold uppercase tracking-wider text-oxblood-dark hover:bg-gold-light transition shadow-lg">Update Stage</button>
                </form>
            </div>
        </div>
        ...
        <div x-show="zoomModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md" @click="zoomModal = false">
            <div class="relative max-w-2xl max-h-[90vh] overflow-auto border border-gold-antique/50 bg-black p-2" @click.stop>
                <img src="{{ asset($order->payment_slip_path) }}" alt="Enlarged Slip" class="w-full h-auto" />
                <button type="button" @click="zoomModal = false" class="absolute top-4 right-4 bg-oxblood-dark text-gold-light p-1 border border-gold-antique/40"><x-heroicon-m-x-mark class="h-6 w-6" /></button>
            </div>
        </div>
        ...
        <div x-show="rejectModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md">
            <div class="w-full max-w-md border border-gold-antique/40 bg-oxblood-dark p-6 shadow-2xl text-ivory-base" @click.outside="rejectModal = false">
                ...
                <form action="{{ route('admin.orders.payment', $order) }}" method="POST" class="space-y-3">
                    ...
                    <div>
                        ...
                        <textarea name="reason" rows="3" placeholder="e.g. Inconclusive reference number, incorrect amount..." class="w-full border border-gold-antique/30 bg-black/40 p-2 text-xs text-ivory-base focus:border-gold-antique focus:outline-none"></textarea>
                    </div>
                    <div class="flex justify-end gap-2">
                        <button type="button" @click="rejectModal = false" class="px-4 py-2 text-xs border border-gold-antique/30 text-ivory-base hover:text-white">Cancel</button>
                        <button type="submit" class="bg-red-800 hover:bg-red-700 px-4 py-2 text-xs font-semibold text-white uppercase">Confirm Rejection</button>
                    </div>
                </form>
            </div>
        </div>
