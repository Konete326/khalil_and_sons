    ...
    <div class="space-y-8 w-full">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gold-antique/20 pb-4">
            ...
            <form action="{{ route('admin.rates.sync') }}" method="POST">
                @csrf
                <button type="submit" class="border border-gold-antique/60 bg-gold-antique/20 hover:bg-gold-antique hover:text-oxblood-dark px-4 py-2.5 text-xs font-semibold uppercase tracking-wider text-gold-light transition shadow-lg flex items-center space-x-2 rounded">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-[#121212] border border-gold-antique/20 rounded-lg p-6 shadow-2xl" x-data="{ tola: 415000, get gram() { return (this.tola / 11.6638).toFixed(2); } }">
                ...
                <form action="{{ route('admin.rates.update') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="karat" class="w-full border border-gold-antique/30 bg-black/60 p-2.5 text-xs text-ivory-base focus:border-gold-antique focus:outline-none rounded">
                    ...
                    <div>
                        ...
                        <input type="number" step="100" name="rate_per_tola" x-model.number="tola" required class="w-full border border-gold-antique/30 bg-black/60 p-2.5 text-xs text-ivory-base focus:border-gold-antique focus:outline-none font-mono rounded" />
                    </div>

                    <div class="p-3 border border-gold-antique/20 bg-black/40 text-[11px] font-mono rounded">
                    ...
                    <button type="submit" class="w-full bg-gold-antique py-2.5 text-xs font-semibold uppercase tracking-[0.2em] text-oxblood-dark hover:bg-gold-light transition shadow-xl rounded">
