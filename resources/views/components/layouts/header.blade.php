...
>
        ...
        <div class="flex h-16 sm:h-20 items-center justify-between">
            <a href="{{ route('home') }}" class="flex items-center space-x-2.5 sm:space-x-3 group">
                ...
                <div class="flex flex-col">
                    <span class="font-serif text-base sm:text-xl font-medium tracking-[0.18em] text-gold-light">
                        KHALIL & SON'S
                    </span>
                    <span class="hidden sm:inline text-[9px] font-sans tracking-[0.25em] text-gold-antique uppercase">
                        Karachi • Since 1991
                    </span>
                </div>
            </a>
            ...
            <div class="flex items-center space-x-3 sm:space-x-5">
                <div class="relative" @click.outside="currencyOpen = false">
                    <button
                        ...
                        class="flex items-center space-x-1 border border-gold-antique/40 px-2 sm:px-2.5 py-1 text-[11px] sm:text-xs font-medium tracking-wider text-ivory-base hover:border-gold-light hover:text-gold-light transition"
                    >
                    ...
                    >
                        <template x-for="curr in ['PKR', 'USD', 'AED', 'SAR', 'GBP']" :key="curr">
                            <button
                                type="button"
                                @click="currency = curr; currencyOpen = false; $dispatch('currency-change', curr); window.notify('Currency set to ' + curr)"
                ...
                <button
                    type="button"
                    @click="window.customConfirm('Would you like to reserve a private viewing salon in Murshid Bazaar?', () => window.notify('Private salon consultation reserved with master goldsmiths.', 'success'), 'Private Salon')"
                    class="hidden md:inline-flex border border-gold-antique/60 bg-gold-antique/10 hover:bg-gold-antique hover:text-oxblood-dark px-3.5 py-1.5 text-[11px] font-semibold uppercase tracking-widest text-gold-light transition"
                >
                    Book Salon
                </button>
                ...
                <button
                    ...
                    class="p-1 text-ivory-base hover:text-gold-light focus:outline-none md:hidden"
    ...
    <div x-show="mobileMenuOpen" x-cloak class="fixed inset-0 z-50 md:hidden" aria-modal="true">
        ...
        <div
            ...
            class="fixed top-0 right-0 bottom-0 flex h-full w-[85vw] max-w-sm flex-col justify-between border-l border-gold-antique/40 bg-oxblood-dark p-6 shadow-2xl z-10 overflow-y-auto"
        >
            <div>
                ...
                <nav class="mt-6 flex flex-col space-y-3 font-serif">
                    ...
                    <a href="{{ request()->routeIs('home') ? '#gold-rates' : route('home') . '#gold-rates' }}" @click="mobileMenuOpen = false" class="text-sm tracking-widest uppercase transition py-2 border-b border-gold-antique/15 text-ivory-base hover:text-gold-light">Live Gold Rates</a>
                ...
                <div class="mt-8">
                    <button
                        type="button"
                        @click="mobileMenuOpen = false; window.customConfirm('Would you like to schedule a private bridal suite viewing at our Murshid Bazaar showroom?', () => window.notify('Private consultation reserved with our master goldsmiths.', 'success'), 'Private Consultation')"
                        class="w-full bg-gold-antique py-3 text-center text-xs font-semibold uppercase tracking-[0.2em] text-oxblood-dark shadow-xl transition hover:bg-gold-light"
                    >
            ...
            <div class="border-t border-gold-antique/30 pt-6 space-y-4">
                <a
                    href="https://wa.me/923001234567?text=Assalam-o-Alaikum%20Khalil%20%26%20Sons%2C%20I%20would%20like%20to%20inquire%20about%20a%20private%20bridal%20consultation."
                    target="_blank"
                    rel="noopener noreferrer"
                    class="flex items-center justify-center gap-2 border border-[#25D366]/40 bg-[#25D366]/10 px-4 py-2.5 text-xs text-[#25D366] hover:bg-[#25D366] hover:text-black transition"
                >
